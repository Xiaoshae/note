...
1. **桥**：泛指具有任意多个端口的交换机。
2. **桥的MAC地址**：一台交换机有一个专用的基准 MAC 地址（通常称为 **Base MAC**、**System MAC** 或 **Backplane MAC**）。
...
7. **根端口**：非根桥可能由多个端口与根桥通信，非根桥到根桥通信最优的端口（根端口）。

8. **指定端口**：向下层交换机转发数据的端口。

9. **阻塞端口**：被阻塞用于备用的端口，该端口**不转发数据帧**（不接受、不发送），但可以**接收并处理STP帧**（仅接受，不发送）。
