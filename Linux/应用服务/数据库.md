...
`CREATE` 语句是 DDL 的基石，它用于在数据库中**创建新**的对象。DDL 本身是 SQL (Structured Query Language) 的一个子集，专门用于定义和管理数据库的结构（“蓝图”），而不是操纵存储在里面的数据（那是 DML，如 `SELECT`, `INSERT`）。
...
`CREATE` 命令可以创建多种数据库对象，其中最核心、最常用的是 **数据库 (DATABASE)** 和 **表 (TABLE)**。



#### DATABASE 数据库

 **CREATE DATABASE** 使用给定名称创建一个数据库。要使用此语句，您需要拥有该数据库的 **CREATE** 权限。 **CREATE SCHEMA** 是 **CREATE DATABASE** 的同义词。
...
如果使用了可选的 **OR REPLACE** 子句，删除现有数据库并重新创建，等价于以下操作：
...
当使用 **IF NOT EXISTS** 子句时，如果指定的数据库已存在，MariaDB 将返回警告而非错误。



**CHARACTER SET 和 COLLATE**

为该数据库下所有新建表提供**默认的字符集和排序规则**。

```sql
CREATE DATABASE czech_slovak_names 
  CHARACTER SET = 'keybcs2' # 字符集
  COLLATE = 'keybcs2_bin';  # 排序规则
```
...
注释的最大长度为 1024 字节。如果注释长度超过此限制，将抛出错误/警告代码 4144。数据库注释还会被添加到 `db.opt` 文件以及 information_schema.schemata 表中。
...
使用 `CREATE TABLE` 语句创建指定名称的表。执行 `CREATE TABLE` 语句需要对表或数据库拥有 CREATE 权限。
...
在其最基本的形式中，`CREATE TABLE`语句提供一个表名，后跟列、索引和约束的列表。默认情况下，表会在默认数据库中创建。使用`database_name.table_name`指定数据库。如果引用了表名，则必须分别将数据库名和表名引用为`database_name`.`table_name`。这对于 CREATE TABLE ... SELECT 特别有用，因为它允许将表创建到包含来自其他数据库的数据的数据库中



如果存在同名的表，将导致错误 1050。使用 IF NOT EXISTS 可抑制此错误并改为发出提示。使用 SHOW WARNINGS 查看提示信息。



使用 `CREATE TABLE` 语句会自动提交当前事务，除非使用了 TEMPORARY 关键字。
...
如果使用了 `OR REPLACE` 子句且表已存在，服务器不会返回错误，而是会删除现有表并用新定义的表替换它。

此语法最初是为了在副本上必须回滚并重复诸如 `CREATE ... SELECT` 等语句时，使复制更加健壮而添加的。
...
如果 `table_name` 之前被 LOCK TABLES 锁定，该语句执行后它将继续保持锁定状态。

如果使用了 `TEMPORARY` 关键字，则仅删除临时表。（使用 DROP TABLE 时，优先删除临时表而非普通表）。
