...
将 **"打印某个表达式的值"** 的命令添加到 **Auto dispay** 列表中，这样每当你的程序停止时，GDB 会自动打印其值。每个**添加到列表的表达式**都会被**分配一个编号来标识它**；要从列表中移除一个表达式，你需要指定这个编号。
...
这种显示方式展示了项目编号、表达式及其当前值。与你手动使用 **x** 或 **print** 请求的显示一样，你可以**指定你喜欢的输出格式**；

实际上，display **根据你的格式说明**决定是使用 **print** 还是 **x** —— 如果你指定了 **'i' 或 's' 格式**，或者**单位大小**，它将使用 **x**；否则它使用 **print**。



**display/fmt expr**

添加一个表达式 **expr** 到自动显示列表中，并指定每次程序停止时以特定格式 **fmt 显示该表达式的值**。

**expr** 可以是任何**合法的 C 表达式**，比如变量、函数调用等，而 fmt 是一个输出格式，它指定了你希望如何格式化输出这个表达式的值。

如果 fmt 中仅包含**显示格式**而**不包括大小或数量（如字节数）**，那么 GDB 会使用 print 命令来显示结果。
...
用于将内存地址 **addr** 添加到自动显示列表中，并且在每次程序停止时按照指定的格式 **fmt** 来**检查和显示该地址的内容**。

当 **fmt** 包含**‘i’或‘s’格式说明符**，或者指定了**单元大小或单元数量**时，GDB 会认为这是一个内存地址并**执行类似 x/fmt addr 的操作**来查看内存内容。这意味着它不仅仅显示一个值，而是**实际从内存中读取数据**。

例如，**display/i $pc** 很有帮助，每在次执行停止时查看**即将执行的机器指令**。

```
(gdb) display/i $pc
```

> **display** 命令不会在你再次按下回车键后重复执行。



**undisplay dnums…** 或 **delete display dnums…**：从**自动显示列表中移除项**。

参数 **dnums** 表示**要删除的项**。它可以是一个单独的显示编号，如 **info display** 显示的**第一个字段所示**；也可以是多个显示编号的范围，例如 **2 - 4**。

> **undisplay** 命令在你按回车键后不会重复执行。（否则你会得到错误信息‘No display number …’）



**disable display dnums…**：禁用编号为 **dnums** 的显示项。

**enable display dnums…**：启用编号为 **dnums** 的显示项。

> **disable 或 enable** 支持**单独的显示编号**和**多个显示编号的范围**。



**display** 显示**列表中表达式（或内存中）的当前值**，就像你的**程序停止时所做的**。

**info display** 打印之前**已设置自动显示的表达式列表**，每个表达式都**带有其项目编号**，但**不显示值**。这包括被**标记为禁用的表达式**。还包括**现在不会显示的表达式**，因为这些**表达式引用了当前不可用的自动变量**。
